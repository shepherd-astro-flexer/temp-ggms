...
const ClientsPagination = ({attendance}) => {
  ...
  return (
    <div className="join mt-16 flex justify-end">
      <button className={`join-item btn ${attendance && "btn-sm bg-base-100"}`} btn onClick={() => navigatePages(currentPage - 1 < 1 ? numberOfPages : currentPage - 1)}>prev</button>
      ...
      <button className={`join-item btn ${attendance && "btn-sm bg-base-100"}`} btn onClick={() => navigatePages(currentPage + 1 > numberOfPages ? 1 : currentPage + 1)}>next</button>
    </div>
  )
}
